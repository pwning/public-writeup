...
const unsigned char byte_202020[] = {
...
int main()
{
    ...
    for (i = 0, j = 0; i < sizeof(byte_202020); i++)
    {
        if (byte_202020[i] == 0x16)
        {
            int length = invSbox[byte_202020[i+1]];
            unsigned char ch = byte_202020[i+2] - (rand() - 48);
            while (length--)
                output[j++] = ch;
            i += 2;
        }
        ...
        {
            output[j++] = byte_202020[i] - (rand() - 48);
        }
    }
    for (i = 0; i < j; i++)
    {

...
alphabet = b'1234567890ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz'
...
def b59encode(v):
    ...
    result = b59encode_int(acc, default_one=False)
...
def b59encode_check(v):
    '''Encode a string using Base59 with a 4 character checksum'''

    digest = sha256(sha256(v).digest()).digest()
    return b59encode(v + digest[:4])


def b59decode_check(v):
    '''Decode and verify the checksum of a Base59 encoded string'''
